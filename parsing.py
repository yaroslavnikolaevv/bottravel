...
class Parsers:
    def __init__(self,fromInput,fromOutput,date):
        ...
        self.date=date
    def threader(self):
        thread1 = Thread(target=get_r,args=(self.fromInput,self.fromOutput,self.date))
        thread2 = Thread(target=get_s, args=(self.fromInput,self.fromOutput,self.date))

        thread1.start()
        thread2.start()
        thread1.join()
        thread2.join()
        return('Расписание на ж/д маршруты:'+str(ur_rzd)+'\n'+'Расписание на авиарейсы:'+str(ur_s7))
        
def get_r(fromInput,fromOutput,date):
    global ur_rzd
